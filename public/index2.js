var tootMap = {
    ...
    map_domain: "tootmap.net",
    default_tag: localStorage.getItem('default_tag') ? localStorage.getItem('default_tag') : "biwakomap",
    client_name: "biwakodonMap",
    ...
    timeline: {
        api_url: "https://us-central1-biwakodonmap-1497003775665.cloudfunctions.net/get",
        limit: 40,
        max_id: "",
        last_date: "",
        toot_list: [],
    ...
    gmap: {
        ...
        createContent: function(latLng) {
            ...
            if (tootMap.api.access_token!=null) {
                ...
                content = '<div class="detailed-status light"><a class="detailed-status__display-name p-author h-card" rel="noopener" target="_blank" href="https://'+tootMap.mstdn_domain+'/@'+tootMap.api.username+'">'
                    ...
                    + '<div><p class="btn btn-primary" id="toot">公開Toot</p>　<p class="btn btn-primary" id="media"><i class="fa fa-camera" aria-hidden="true"></i></p>　<p class="btn btn-primary" id="staticImage"><i class="fa fa-map-marker" aria-hidden="true"></i></p><input type="file" name="media"></div>'
    ...
    api: {
        ...
        openAuthWindow: function() {
            if (tootMap.api.code==null) {
                location.href = "https://"+tootMap.mstdn_domain+"/oauth/authorize?"+"client_id="+tootMap.api.client_id+"&response_type=code&redirect_uri=https://"+tootMap.map_domain+"/index2.html&scope=read%20write";
            }
        },
        getClientInfo: function() {
            var url = "https://"+tootMap.mstdn_domain+"/api/v1/apps";
            var data = {client_name: tootMap.client_name, redirect_uris: "https://"+tootMap.map_domain+"/index2.html", scopes: 'read write'};
            var xhr = new XMLHttpRequest();
            xhr.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    if (this.responseText.length == 0) {
        ...
        getAccessToken: function() {
            ...
            var data = {grant_type: "authorization_code", redirect_uri: "https://"+tootMap.map_domain+"/index2.html", client_id: tootMap.api.client_id, client_secret: tootMap.api.client_secret, code: tootMap.params.code};
            var xhr = new XMLHttpRequest();
            xhr.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    if (this.responseText.length == 0) {
        ...
        attachStaticMapImage: function(position, zoom) {
            var api_key = "AIzaSyDZcAYOeC2U-f1szMIykGHfUTwki09BSko"
            var height = 640;
            var width = 400;
            var url = "https://maps.googleapis.com/maps/api/staticmap?center="+position.lat()+","+position.lng()+"&zoom="+zoom+"&size="+height+"x"+width+"&key="+api_key;
            var xhr = new XMLHttpRequest();
            xhr.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    tootMap.api.mediaUpload(this.responseText);
                }
            };
            xhr.open("GET", url, true);
            xhr.send();
    ...
    createMenuDiv: function() {
        ...
        $("#setting").html(
            ...
            + "<a class='btn btn-block btn-light' target='_blank' href='./howtouse.html'>使い方はこちら</a>"
...
//        tootMap.api.attachStaticMapImage(new google.maps.LatLng(37.530101,38.600062), 13);
        ...
        $("body").on("click", "#staticImage", function() {
            tootMap.api.attachStaticMapImage(tootMap.gmap.open_window.getPosition(), tootMap.gmap.map.getZoom());
        })
    }
};